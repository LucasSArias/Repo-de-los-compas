...
cd /home/utnso/tp-2024-2c-Task-Managers/Pruebas_pseudocodigo/valgrind

valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=filesystem-valgrind-out.log "${RUTA_FILESYSTEM}bin/${MODULO_FILESYSTEM}" "${RUTA_FILESYSTEM}${MODULO_FILESYSTEM}${PRUEBA}" &
sleep 2
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=memoria-valgrind-out.log "${RUTA_MEMORIA}bin/${MODULO_MEMORIA}" "${RUTA_MEMORIA}${MODULO_MEMORIA}${PRUEBA}" &
sleep 2
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=cpu-valgrind-out.log "${RUTA_CPU}bin/${MODULO_CPU}" "${RUTA_CPU}${MODULO_CPU}${CONFIG}" &
sleep 2
valgrind --leak-check=full --show-leak-kinds=all --track-origins=yes --log-file=kernel-valgrind-out.log "${RUTA_KERNEL}bin/${MODULO_KERNEL}" "${RUTA_KERNEL}${MODULO_KERNEL}${PRUEBA}" "${FILENAME}" "${TAM_PROCESO}" &